...
⚠️ **Talk to your mentor — in week 1, not week 9.** Four questions to ask:
...
4. *"By the end of the summer, **which parts of this project should I be
...
can't yet tell which norms are universal vs. lab-specific. Question 4 is
the one mentors will love them for asking — it defines the learning goals
up front and previews Part 6 (the delegation boundary: you can only
delegate what you could evaluate). The data question previews the Ground
rules slide near the end.
:::
...
**✗** The summary said *"works perfectly"* — over three unexamined warnings.

> "Every report ends with a **remaining-uncertainty** section:
> what was assumed, what wasn't checked."

::: {.notes}
~3 min. This is the single most important habit in the talk — say so. The
...
Also in the repo: $slurm and $job-babysitting for cluster work — relevant
to anyone whose REU project touches the RCC clusters. Keep shared skills
...
> is reported**. At the top: one line on **what this run tests**. Then enough
> to rerun or audit it: the exact **command**, the **config**, the **input
> files**, the **git commit**, and **how to regenerate it**. Add key plots
> and numbers as they appear."
...
the rule is in AGENTS.md: from then on every result directory documents
itself, and "reported as ready" is gated on the README existing. ("Before
the result is reported" is the load-bearing phrase — it makes provenance
part of the definition of done, not cleanup.) This discipline is rare even
among professional scientists; a student who has it stands out immediately.
:::
...
You've seen this move twice now — the wrong-file rule, the seed rule.
...
You cannot list all the failure modes in advance. Don't try — **harvest them**.
...
::: {.notes}
~2 min. Explicit callback to "Where the rules come from" in Part 4 — name
it as the same single habit so it lands as one move, not two slides.
Reframes errors as compounding assets, and answers "the agent doesn't
learn between sessions": right — so the learning lives in the repo. This
flywheel is also exactly how labs onboard humans; the analogy holds.
:::
