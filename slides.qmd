...
**If your work is computational experiments**, it scales on the cluster: the agent
writes the **SLURM** script, launches, and files each run in its own provenance'd
folder — **dozens of runs, each self-documenting**, while you sleep.
...
non-modeling students see themselves. The SLURM line is scoped on purpose —
a highlight for the modeling/simulation half (CNNs, simulation-based
inference, materials/astro) and a fun aside for the data-engineering / tool
/ analysis half; don't imply everyone should be running sweeps. The agent
can run a whole campaign on the RCC cluster — write the submit script,
launch, babysit the queue, pull results back — each run landing in its own
README'd folder ($slurm / $job-babysitting skills). This discipline is rare
even among pros; a student who has it stands out.
:::
