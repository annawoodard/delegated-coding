...
the remaining lever is cutting "three generations", ~-2 min):
  ...
  ~18  part 4: running the lab (8 practices: added checkable-target + lab-notebook)
...
One experiment → **one config** → **one directory** → a `README` of how it was made.
...
On the cluster this is where it pays off: the agent writes the **SLURM** script,
launches the run, and drops it in its own provenance'd directory —
**dozens of experiments, each self-documenting**, while you do something else.
...
part of the definition of done, not cleanup.) The HPC line is the exciting
one for this audience: the agent can run a whole campaign on the RCC cluster
— write the submit script, launch, babysit the queue, pull results back —
each run landing in its own config'd, README'd directory. That's the
$slurm / $job-babysitting skills from the skills slide. This discipline is
rare even among professional scientists; a student who has it stands out.
:::
...
- a run's **README** = how *one* experiment was made *(provenance)*
...
> "After each experiment, append to `LAB_NOTEBOOK.md`: what you tried, the
> result, and what it changes about **what we believe** and **try next**."
...
::: {.notes}
~2 min. The distinction is the teaching point: the README answers "how do I
