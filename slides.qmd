...
::: {.notes}
~1.5 min. TITLE-SLIDE OPENER — say this over the title slide as you walk out,
before advancing: "This talk was prepared the way it recommends. I sketched the
argument; my agent drafted the slides — I own all the claims." That's the whole
hour in miniature: the agent did the typing, I did the thinking and the
reviewing. If anything here is wrong, that's on me, not the tool. Then advance.

INTERACTIVE (this slide): "Raise your hand if you think genAI is good for the
world. Now raise your hand if you think it's bad. … If both hands went up,
good — that's the honest answer." Read the room; if it's stiff, skip the hands
and just say it.

Why this slide exists: a lot of you have real, considered objections — that's
why people boo it at commencement. I won't stand here and pretend it's
uncomplicated. The costs are real, the benefits are real, and nobody has fully
resolved this. Today is deliberately narrower: not "should this exist," but
"given that it does and you may use it, how do you do that well." We hit the
career version of the worry head-on near the end (the "take my job" slide).
:::
...
slide — see its DO-NOT-CUT note.) On "auto-compact": Claude Code and Codex
now summarize the old context automatically when the window fills, so the
session doesn't hard-stop — but compaction is lossy and you don't pick what
survives, so it actually REINFORCES the thesis: don't rely on its memory,
within OR across sessions; the record you'll defend belongs in the repo.
Starting a fresh session for a fresh task is still cleaner than working from
a compacted one. On cross-session "memory": yes, Claude Code and ChatGPT now
persist some — say so if asked, but same point: partial, opaque, not
versioned or auditable. One sentence each; don't belabor it.
:::
...
::: {.notes}
~3 min. For students who've never done research, this is the framing that
matters most: there's no answer key. A bug that crashes is a good bug — it
announces itself. The dangerous bug runs clean and quietly changes the
result, and nothing automatic warns you — so "it works" means something
different from here on, and the checking becomes human judgment (yours
first, then your mentor's, eventually a reviewer's). Don't oversell "only
you" — the point is there's no autograder, not that you're alone. Sets up
Part 5 (failures that look right) and Part 4 (the provenance + lab-notebook
habits that make wrong claims catchable). The "you'll redo everything"
motivation for reproducibility now lives where it belongs, on the auditable
slide in Part 4.
:::
...
::: {.notes}
~3 min. The reframe (Anna's point): the talk's real enemy is the RIGHT
failure — vibe-delegating, shipping what you can't vouch for (slide 2, and
all of Part 6). Micromanaging is the opposite over-correction: you keep
every detail but gain no leverage. Good delegation threads between — you
hand off the typing, NOT the understanding. The delegating prompt does five
jobs: goal, acceptance criteria, process (inspect → plan → implement →
verify), scope ("smallest thing"), reporting — copy-pasteable, say so. Land
the closing hard: "I didn't type it, but I understand it and can defend it"
is the whole talk in a line. The two compressed phrases here — "what would
convince me" and "propose a plan" — each get their own slide next.
:::
...
::: {.notes}
~2-3 min. Unpacks the "what would convince me" phrase from the prompt shift —
Anna's note: the abstract phrase needs concrete examples to land. The move:
replace a vague verb ("make", "clean", "speed up") with an observable test.
INTERACTION: show the vague column, then read the "Your turn" prompt and
actually take 2-3 answers from the room out loud (it doubles as a domain
check — naming analysis/simulation/imaging/scraping lets the non-medical
students see themselves). THEN click to reveal the right column as "here's
how I'd make those checkable." Don't reveal early — the pause is the point,
and the answers tell you how statsy/ML the room is. If they're stuck, hand
them the first one ("make a plot" -> "plots x vs y, one series per group,
axes labeled"). That plot criterion needs NO reference figure — the
spec itself is the check; if you happen to have one, "reproduces Fig. 2 within
rounding" also works. The model one operationalizes the weasel word "good":
a pre-set metric margin (≥0.02 AUC — swap in your metric) AND robustness to a
new seed. Each right-column example seeds something later — "same outputs, just
faster" is the refactor invariant; "survives a new seed" is the Part 5 seed
story; the row-count one is the silent-N failure (kept the 412 but dropped
"patients" so it reads for any dataset). This is the single highest-leverage
habit in the talk — say so. It's also the input to practice 4 (give it a
checkable target): a runnable acceptance criterion IS the target the agent
iterates against.
:::
...
::: {.notes}
~2 min. Unpacks "propose a plan" and elevates it to a habit, because it's
one of the highest-leverage things they'll learn: the cheapest place to
catch a misunderstanding is BEFORE the code exists. A plan is a paragraph
you read in a minute; 300 lines of confidently-wrong code is an hour you
won't get back. The three checks are the review surface — especially
"assumptions match your data," where the model's plausible-but-wrong guesses
hide (under-the-hood slide). For a big task, have it write the plan to a file
(DESIGN.md) and commit it, so the implementation can be checked against what
you agreed to. Pairs with "demand the full loop": the plan is the front
gate, the report is the back gate.
:::
...
::: {.notes}
~3 min. Many will know git from class; some won't. The reframe: git isn't a
chore, it's the safety net that makes delegation safe — you can revert
anything and trace any result to the commit that made it. Don't sell
diff-reading as required review (Anna doesn't read her code); if a student
finds reading diffs helps them learn, great — but the real verification is on
the outputs (Part 5). Agents write good commit messages — make them do it;
those commits are also your provenance trail (links to the auditable practice).
:::
...
::: {.notes}
~3 min. All four are real, lightly trimmed, from ONE file — my HFDP
project's AGENTS.md (~100 rules grown over a year). Nearly every rule is a
scar: something went wrong once, I caught it, and the general version went
in the file (the flywheel, Part 5). Point out the range — it climbs from
code hygiene (fail fast) and comparison hygiene (one knob) up to scientific
judgment: only run decision-worthy experiments, and never let a convenient
shortcut quietly corrupt the result (that last one is the Part 5 thesis as a
config rule). The provenance/README rule lives on the auditability slide
instead — deliberately not said twice. A favorite cut for
space: "When comparing runs, state the shared setup and the differing knob
before the takeaway" — reports arrive reviewable. The agent reads all of it
every single session. Theirs starts at ten lines; this is what it compounds
into.
:::
...
::: {.notes}
~3 min. The technical point to land (Anna's): a skill is a folder whose
header is a tiny title + description — those few tokens are always in
context, so the agent knows the skill EXISTS; the full body (often long)
loads only when the description matches the task. That's "progressive
disclosure" — the reason you DON'T hand-invoke them (the old "type
$sensemaking" move): you make them available, the agent self-selects.
Payoff: the same vetted workflow fires every conversation instead of the
model improvising a new one, and you're not paying tokens to paste the whole
workflow into every prompt. The arc completes here — a rule fixes one
mistake, AGENTS.md compounds rules for one project, skills package what
generalizes ACROSS projects (the list is this talk in tool form). Install =
clone the repo, copy/symlink folders into the skills dir (~/.claude/skills,
~/.cursor/skills, ...); README has per-tool commands. Also in the repo:
$experiment-design, $rut-breaker, and $slurm / $job-babysitting for cluster
work. Keep shared skills generic; project facts stay in AGENTS.md.
:::
...
::: {.notes}
~3 min. The motivation — say it aloud: you WILL redo everything (the data
gets updated, a week-7 bug, the mentor asks "what if we exclude those
samples?"), so a result you can't regenerate is one you can't trust;
reproducibility here is self-defense, not an ideal. The agent
...
It's the physical home of the whole thesis: the context-window slide says you
can't rely on its memory; the PI slide says it won't carry lessons unless you
write them down; the flywheel says the learning lives in the repo. THIS is
where it lives. Slow down here; it deserves more time than its ~2 min, not less.
...
prompt. Push the STUDENT payoff hard, not just the agent's memory: it stops
you re-running dead ends (you can see what you already tried and ruled out),
and it's what you scroll back through to prep a mentor meeting — "here's what
I did and what I found." Same record, three jobs: the agent's memory, your
anti-circles log, your mentor-update / poster skeleton. Mine doubles as the skeleton of every update I send collaborators
...
(practice 5) interlock. (2) The **Surprise** line is the gold: an anomaly
...
it works is that it's the SYNTHESIS of the practices before it: the success
metric IS the checkable target (practice 4); each run self-documents
(practice 5); the notebook logs the thread (practice 6). Strip those away
...
these four is a failure mode I have actually hit. The point (Anna's): don't
counsel them to read the code — you probably won't, and these bugs aren't IN
the code anyway, they're in what it produced. Each maps to a check: rows →
counts; wrong file → the path actually loaded; hidden assumption → the
config; overconfident summary → the wording. That's the make-auditing-cheap
checklist two slides on.
:::
...
assets, and answers "can I rely on its memory?": no — so the learning lives
in the repo, where you control and version it; the thing real labs take years
to build.
:::
...
::: {.notes}
~3 min. Anna's point: "only delegate what you can evaluate" is circular if you
don't know what you can't evaluate. The resolution is the method on the slide —
you don't have to spot blind spots cold; you make the agent ENUMERATE its
assumptions, decisions, and failure modes (it's good at this — it knows the
standard caveats even when you don't), and your gaps surface as the choices you
didn't know existed and the explanations you can't follow. "Could I defend this
to my mentor?" is the gut check: if you'd go hand-wavy, that's the gap. This
converts unknown-unknowns into known-unknowns, which you then learn (next slide:
make it a tutor) or escalate to your mentor — and running that loop is exactly
how the boundary moves. Domain-swap the t-test if the room isn't statsy:
"95% accuracy" on imbalanced data, or a join that silently drops rows.
:::
...
is the thing you never do with sensitive data. On the code bullet: don't
imply unpublished = forbidden. Most research code is unpublished — that's the
normal case, not a red flag. The only rule is get the mentor's OK before
sending the project's source to a cloud tool (a promise I made to a mentor
whose student is on not-yet-published code). Make the carve-out loud or they
hear "no AI at all": they can still use it as a tutor/planner/debugger
WITHOUT the source — explain a concept, debug from the error text, sketch
pseudocode, plan the workflow. And mentor rules override me, always. The
accountability point closes the loop with the opener: I own every claim on
these slides.
:::
