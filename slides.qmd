...
STATUS: first trim pass done (cut: both-sides segue [merged into mental
model], resources slide [links now a footer on the takeaways slide];
workflow-maintenance slide kept by request). Rough timing (~67 min —
still ~7-12 over if Q&A stays inside the hour):
  ~5   hook + reframe
  ~15  part 1: what you're working with (LLMs, agents, harnesses)
  ~5   part 2: research is a different game
...
Next trim candidates if a dry run confirms it's over: "You've probably met
three generations of this" (part 1) and "What a session actually looks
like" (part 1) — both are color, not load-bearing.
