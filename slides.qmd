...
slide. ★ DO NOT CUT — the lab-notebook slide (practice 6) is, in Anna's
...
slide — see its DO-NOT-CUT note.) On "memory": yes, Claude Code and ChatGPT
now persist some memory across sessions — say so if asked, but the point
holds: that memory is partial, opaque, and not versioned or auditable, so the
record you'll defend in a paper belongs in the repo where you control it. One
sentence; don't belabor it.
:::
...
- "reproduces Fig. 2 within rounding"
- "row count = 412 patients, or tell me why not"
- "**same outputs**, just faster"
- "beats the baseline — and **survives a new seed**"
:::
:::
...
::: {.notes}
~2 min. Unpacks the "what would convince me" phrase from the prompt shift —
...
Each right-column example seeds something later — "same outputs, just
faster" is the refactor invariant; "survives a new seed" is the Part 5 seed
story; the row-count one is the silent-N failure. This is the single
highest-leverage habit in the talk — say so. It's also the input to practice
4 (give it a checkable target): a runnable acceptance criterion IS the
target the agent iterates against.
:::
...
delegate boldly. *(Reading the diff is a fine way to learn what changed; the real check
is the **outputs**, not the lines — Part 5.)*
...
::: {.notes}
~3 min. All five are real, lightly trimmed, from ONE file — my HFDP
...
code hygiene (fail fast) and comparison hygiene (one knob) and provenance
(the README) up to scientific judgment: only run decision-worthy
experiments, and never let a convenient shortcut quietly corrupt the result
(that last one is the Part 5 thesis as a config rule). A favorite cut for
...
**counts**, the **file actually loaded**, the **config**, the **summary's wording**.
