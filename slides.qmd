...
> "Retrieval can find a passage — it is **not evidence**."
> [— read the source yourself before you cite it]{.smallgray}

> "For every source, note what it **does not prove**."
> [— the caution has to travel with the claim]{.smallgray}

::: {.notes}
~3 min. These are real, lightly trimmed — pulled from my radiogenomics
agents (the imaging pipeline and the review-article repo). Nearly every
rule is a scar: something went wrong once, I caught it, and the general
version went in the file (the flywheel, Part 5). Point out the range — code
hygiene, comparison hygiene, provenance, verifying sources, refusing to
overclaim — and that the last two ARE this whole talk in two lines: a
retrieved passage isn't evidence, and every claim carries what it doesn't
prove (callback to the under-the-hood and remaining-uncertainty slides). The
agent reads all of it every single session. Theirs starts at ten lines;
this is what it compounds into.
:::
